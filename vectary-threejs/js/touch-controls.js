...
function TouchControls(container, camera, options) {
  ...
  var ztouch = 1,
    xtouch = 1;
  var PI_2 = Math.PI / 2;
  var maxPitch = (self.config.maxPitch * Math.PI) / 40;
  ...
  function onMouseMove(event) {
    ...
    var movementY =
      ...
      0;
    var rotation = calculateRotation(-1 * movementX, -1 * movementY);
  ...
  function calculateRotation(dx, dy, factor) {
    var factor = factor ? factor : self.config.rotationFactor;
    var ry = self.object.rotation.y + dx * factor;
    var rx = self.object.rotation.x + dy * factor;
  ...
  self.update = function () {
    ...
    self.object.translateX(velocity.x);
    self.object.translateY(velocity.y);
    self.object.translateZ(velocity.z);
  };

  self.hitTest = function () {
    self.unlockAllDirections();
    hitObjects = [];
    var currentDirection = self
      .getDirection2(new THREE.Vector3(0, 0, 0))
      .clone();

    for (var i = 0; i < 4; i++) {
      // Applying rotation for each direction:
      var direction = currentDirection.clone();
      direction.applyMatrix4(rotationMatrices[i]);

      var rayCaster = new THREE.Raycaster(self.object.position, direction);
