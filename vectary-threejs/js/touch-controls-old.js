...
function TouchControls(container, camera, options) {
  ...
  self.getDirection2 = function (v) {
    ...
    var rx = self.fpsBody.getObjectByName("cameraHolder").rotation.x - 1;
...
TouchControls.prototype = {
  ...
  setCamHolder: function () {
    var cameraHolder = this.fpsBody.getObjectByName("cameraHolder");
    return this.cameraHolder
  },

  setRotation: function (x, y) {
    // var camHolder = this.fpsBody.getObjectByName("cameraHolder");
    var camHolder = this.fpsBody
    // works at eliminating Y plane at least
    if (x !== null)
